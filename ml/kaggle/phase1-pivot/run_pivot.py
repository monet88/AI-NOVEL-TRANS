"""Self-contained Kaggle runner for Phase 1 — Pivot ZH→EN via Gemini Flash-Lite.
...
Set DIRECT_GEMINI_API_KEY below in the private Kaggle notebook before running.
This file intentionally embeds the pivot implementation because Kaggle script
kernels may not place sibling files such as pivot_zh_en.py on sys.path.
"""
...
# ---------------------------------------------------------------------------
# Kaggle runtime entrypoint
# ---------------------------------------------------------------------------

# Install runtime dependencies in the Kaggle environment.
...
# Private notebook workaround: paste the real key in Kaggle UI only.
...
os.environ["GEMINI_API_KEY"] = DIRECT_GEMINI_API_KEY.strip()

DATASET_FILENAME = "tran_vi_teacher_strict_clean_dedup_source.jsonl"
EXPECTED_INPUT_JSONL = Path("/kaggle/input/en-vi-novel-mt-raw") / DATASET_FILENAME


def find_input_jsonl() -> Path:
    if EXPECTED_INPUT_JSONL.exists():
        return EXPECTED_INPUT_JSONL

    matches = sorted(Path("/kaggle/input").glob(f"**/{DATASET_FILENAME}"))
    if matches:
        print(f"Using dataset file: {matches[0]}")
        return matches[0]

    available = sorted(str(path) for path in Path("/kaggle/input").glob("*"))
    raise FileNotFoundError(
        "Dataset file not found. Attach Kaggle dataset "
        "minhthang6789/en-vi-novel-mt-raw to this notebook. "
        f"Expected {EXPECTED_INPUT_JSONL}. Available inputs: {available}"
    )
